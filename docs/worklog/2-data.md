...
- เช็คฐานจริง `services/backend/instance/luma.db` พบว่าค้างที่ `18566175f613` (migration ตัวแรก)
  ...
  แปลว่าถ้ารันเว็บทดสอบ login ด้วยมือจะพัง
  · **แก้คำพูดตัวเองรอบนี้**: ตอนแรกอ้างว่า "ทดสอบ #97 ไม่ได้เพราะคอลัมน์ยังไม่มี" — ผิด
    `test_users_table.py:30-43` สร้าง `.db` ชั่วคราวใน `tmp_path` ตั้ง `LUMA_DATABASE_URI`
    แล้วรัน `upgrade()` เองทุกครั้ง ไม่แตะ `instance/luma.db` → เขียน/รัน test ได้ไม่เกี่ยวกับฐานในเครื่อง
...
**ผลตรวจรอบสอง — คิวงานที่ผมแนะนำไปผิดลำดับ**
- **#17 ต้องมาก่อน #97** — #17 เป็น `priority:high` ส่วน #97 เป็น `priority:medium`
  และ #49 ที่ยังเปิดอยู่ระบุเองว่า *"ต้องรอ #17 (UNIQUE INDEX แบบไม่สนตัวพิมพ์) จากคนที่ 2"*
- ตรวจแล้วว่า #17 ยังไม่ได้ทำจริง: `deba60c08f36` สร้าง `username`/`email` เป็น `sa.String`
  + `UniqueConstraint` **ไม่มี `COLLATE NOCASE`** · `auth.py:70` กันซ้ำด้วย `User.query.filter(...)`
  ใน Python = race condition ที่ START_2_DATA เรียกว่างานสำคัญที่สุดของตำแหน่งนี้
  · `auth.py:59` ทำ `.lower()` ให้แค่ `email` ส่วน `username` ไม่ถูก normalize
    → ตอนนี้ `Boss` กับ `boss` สมัครเป็นสองบัญชีได้ บน develop จริง
- **เจอของที่จะพังตอนทำ #97 ที่ยังไม่มีใครจด**: `test_assets_table.py` มี 3 ฟังก์ชันที่ INSERT
  `Asset` โดยไม่ใส่ `user_id` (บรรทัด 62, 88-90, 119) ทั้งหมดจะล้มทันทีที่บังคับ `NOT NULL`
  → ต้องแก้ให้สร้าง `User` ก่อนแล้วส่ง `user_id` **ในPR เดียวกับ migration**
